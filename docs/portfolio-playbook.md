...
PostgreSQL CDC, 애플리케이션 이벤트, crypto tick stream을 Kafka로 통합하고, Flink 스트리밍 처리로 Iceberg 레이크하우스에 적재한 뒤 Trino, Superset, Great Expectations까지 연결한 end-to-end 실시간 데이터 플랫폼입니다.
...
- 고빈도 market tick stream과 event-time window 집계 설계
...
- Kafka는 crypto tick stream도 함께 수용해 다중 도메인 스트리밍 버스로 동작합니다.
...
- crypto tick처럼 고빈도 이벤트 스트림을 붙이면 대용량 실시간 처리 시나리오까지 설명할 수 있습니다.

### Why Flink SQL

- 스트리밍 조인과 윈도우 집계를 SQL 중심으로 표현할 수 있습니다.
- checkpoint 기반 장애 복구와 exactly-once 이야기를 하기 좋습니다.
- crypto tick 버전에서는 1초 micro-batch와 1분 KPI window를 함께 설명할 수 있습니다.
...
CDC와 이벤트 스트리밍을 같은 Iceberg 저장 계층으로 수렴시켰다는 점입니다. Postgres 변경은 Debezium으로, 앱 이벤트와 crypto tick stream은 Schema Registry 기반 Avro로 Kafka에 넣고, Flink SQL이 이를 bronze / silver / gold로 정제합니다. 이렇게 하면 배치와 스트리밍이 같은 테이블 포맷 위에서 만날 수 있고, Trino와 Superset은 같은 Iceberg 테이블을 바로 조회할 수 있습니다.
...
- Extended the platform with crypto tick streaming topics and event-time market KPI aggregations designed for large-scale benchmark scenarios up to 100M rows.
