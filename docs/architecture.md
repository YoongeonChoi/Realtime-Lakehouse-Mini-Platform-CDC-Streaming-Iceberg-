...
4. crypto tick producer는 Avro + Schema Registry 조합으로 `raw.event.market.crypto_ticks_v1` 토픽에 적재합니다.
5. Flink SQL은 Kafka source를 읽어 bronze / silver / gold Iceberg 테이블에 쓰기 작업을 수행합니다.
6. Trino와 Superset은 같은 Iceberg 카탈로그를 통해 조회합니다.
7. Great Expectations는 Trino를 통해 gold 테이블을 검증하고 Data Docs를 생성합니다.
